<div align="center">

<img src="Assets/Banner.png" alt="Somayeh Forouzandeh Banner" width="100%" />

</div>

<h3 align="center">
Industrial Engineer (M.Sc.) | Data & AI | Business Intelligence
</h3>

<p align="center">
Business Intelligence • Data Analytics • Data Science • Machine Learning • Deep Learning • Computer Vision
</p>

<p align="center">
Industrial Engineer with a Master's degree in Systems & Productivity, combining engineering thinking with Data, Artificial Intelligence, and Business Intelligence to transform data into insights, predictive solutions, and smarter decisions.
</p>

<div align="center">

<a href="https://github.com/somayehforouzandeh">
<img src="https://img.shields.io/badge/GitHub-somayehforouzandeh-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://www.linkedin.com/in/somayeh-forouzandeh">
<img src="https://img.shields.io/badge/LinkedIn-Somayeh%20Forouzandeh-0A66C2?style=for-the-badge&logo=linkedin"/>
</a>

</div>

---

# 👋 About Me

🎓 **MSc Industrial Engineering – Systems & Productivity**

📊 **Business Intelligence & Data Analytics**

🔬 **Data Science & Exploratory Data Analysis**

🤖 **Artificial Intelligence & Machine Learning**

🧠 **Deep Learning & Artificial Neural Networks**

👁️ **Computer Vision & Image Processing**

🏭 **Industrial Engineering, Process Improvement & Performance Management**

📈 **Project Planning & Control**

I am an **Industrial Engineer with a Master's degree in Systems & Productivity**, passionate about the intersection of **Industrial Engineering, Data, Artificial Intelligence, and Business Intelligence**.

I combine engineering-based problem-solving and systems thinking with modern data and AI technologies to:

- Transform raw data into actionable insights
- Discover patterns and relationships in data
- Develop predictive and machine learning solutions
- Apply neural networks to real-world prediction problems
- Explore computer vision applications
- Design business intelligence dashboards
- Analyze and improve processes
- Support data-driven decision-making

My professional and technical interests span across:

```text
Industrial Engineering
        ↓
Business Intelligence
        ↓
Data Analytics
        ↓
Data Science
        ↓
Machine Learning
        ↓
Deep Learning & Neural Networks
        ↓
Artificial Intelligence & Computer Vision
```

---

# 🎯 Core Expertise

## ⚙ Industrial Engineering

- Systems & Productivity
- Process Improvement
- Process Optimization
- Performance Management
- KPI Design
- Lean Six Sigma
- EFQM
- Balanced Scorecard (BSC)
- Continuous Improvement
- Data-driven Process Analysis

---

## 📊 Business Intelligence & Data Analytics

- Microsoft Power BI
- Power Query
- DAX
- Data Modeling
- Dashboard Design
- KPI Development
- Business Reporting
- Exploratory Data Analysis (EDA)
- Data Visualization
- Data-driven Decision Making

---

## 🔬 Data Science

- Data Cleaning
- Data Preprocessing
- Exploratory Data Analysis
- Feature Engineering
- Statistical Analysis
- Predictive Analytics
- Classification
- Regression
- Model Evaluation
- Data Visualization

---

## 🤖 Artificial Intelligence & Machine Learning

- Artificial Intelligence
- Machine Learning
- Supervised Learning
- Classification
- Regression
- Predictive Modeling
- Feature Engineering
- Model Training & Evaluation

---

## 🧠 Deep Learning & Neural Networks

- Deep Learning Fundamentals
- Artificial Neural Networks (ANN)
- Neural Network-based Prediction
- Model Training
- Predictive Modeling
- Model Evaluation

---

## 👁️ Computer Vision

- Computer Vision
- Image Processing
- Image Classification
- Image-based Machine Learning Applications
- OpenCV

---

## 📅 Project Management

- Primavera P6
- Project Planning & Control
- Scheduling
- Progress Monitoring
- Project Performance Analysis

<p>
<img src="https://img.shields.io/badge/Primavera%20P6-005BAC?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Project%20Planning-198754?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Project%20Control-6f42c1?style=for-the-badge"/>
</p>

---

# 🛠 Tech Stack

## ⚙️ Industrial Engineering

<p>
<img src="https://img.shields.io/badge/Industrial%20Engineering-0B5394?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Systems%20%26%20Productivity-1E88E5?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Process%20Improvement-198754?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Performance%20Management-6f42c1?style=for-the-badge"/>
<img src="https://img.shields.io/badge/KPI%20Design-F39C12?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Lean%20Six%20Sigma-009688?style=for-the-badge"/>
<img src="https://img.shields.io/badge/EFQM-8E44AD?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Balanced%20Scorecard-34495E?style=for-the-badge"/>
</p>

---

## 📊 Business Intelligence

<p>
<img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
<img src="https://img.shields.io/badge/DAX-FFC107?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Power%20Query-0F9D58?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Data%20Modeling-1976D2?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Dashboard%20Design-1565C0?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Microsoft%20Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white"/>
</p>

---

## 📅 Project Management

<p>
<img src="https://img.shields.io/badge/Primavera%20P6-005BAC?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Project%20Planning-198754?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Project%20Control-6f42c1?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Scheduling-8E44AD?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Progress%20Monitoring-546E7A?style=for-the-badge"/>
</p>

---

## 🐍 Programming & Data Analytics

<p>
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge"/>
</p>

---

## 🗄 Database

<p>
<img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white"/>
</p>

---

## 🤖 Artificial Intelligence & Machine Learning

<p>
<img src="https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge&logo=scikitlearn&logoColor=orange"/>
<img src="https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>
<img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white"/>
<img src="https://img.shields.io/badge/Keras-D00000?style=for-the-badge&logo=keras&logoColor=white"/>
</p>

---

## 🧠 Deep Learning & Neural Networks

<p>
<img src="https://img.shields.io/badge/Deep%20Learning-412991?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Neural%20Networks-FF6F00?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Artificial%20Neural%20Networks-6A1B9A?style=for-the-badge"/>
</p>

---

## 👁️ Computer Vision

<p>
<img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white"/>
<img src="https://img.shields.io/badge/Computer%20Vision-1976D2?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Image%20Processing-00897B?style=for-the-badge"/>
</p>

---

## 🧰 Development Tools

<p>
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
<img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white"/>
<img src="https://img.shields.io/badge/PyCharm-000000?style=for-the-badge&logo=pycharm&logoColor=white"/>
<img src="https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white"/>
</p>

---

# 🚀 Featured Projects

## 📊 Power BI Sales Management Dashboard

Interactive Business Intelligence dashboard designed for sales analysis, KPI monitoring, and management reporting.

**Focus Areas:**

`Business Intelligence` `Data Modeling` `DAX` `Power Query` `KPI Analysis`

---

## 🔬 JMP Data Analytics & Data Storytelling

Exploratory data analysis project focused on identifying demand patterns, user behavior, and environmental factors through statistical analysis and data visualization.

**Focus Areas:**

`Data Analytics` `EDA` `Data Visualization` `Statistical Analysis`

---

## 🧠 Neural Network-Based Credit Limit Prediction

Machine learning project focused on predicting credit limits using a neural network-based approach.

**Focus Areas:**

`Artificial Intelligence` `Machine Learning` `Deep Learning` `Neural Networks` `Predictive Modeling`

---

## 👁️ Smile Detection Using OpenCV and SVM

Computer vision and machine learning project for smile detection using image processing techniques and Support Vector Machines.

**Focus Areas:**

`Computer Vision` `OpenCV` `Image Processing` `SVM` `Machine Learning`

---

## 🎓 University Management System

Python-based desktop application for managing students, courses, grades, academic records, and PDF reports.

**Focus Areas:**

`Python` `Tkinter` `JSON` `ReportLab` `OOP` `GUI Development`

---

## 🐍 Python Data Analytics Projects

Practical Python projects focused on data processing, data analysis, visualization, automation, and real-world problem solving.

---

# 📚 Current Focus

I am continuously developing my skills in:

- Advanced Python for Data Analytics and Data Science
- Machine Learning Applications
- Deep Learning & Neural Networks
- Artificial Intelligence
- Computer Vision
- Business Intelligence Solutions
- Data-driven Process Improvement

---

# 🧩 My Professional Perspective

I believe that the most valuable solutions are created where different disciplines meet.

```text
Industrial Engineering
        +
Business Intelligence
        +
Data Analytics
        +
Data Science
        +
Artificial Intelligence
        ↓
Practical Data-Driven Solutions
```

My background in **Industrial Engineering** helps me understand systems, processes, productivity, and organizational performance.

My skills in **Business Intelligence, Data Analytics, Data Science, Machine Learning, Deep Learning, and Computer Vision** help me analyze data, discover patterns, develop predictive solutions, and support better decision-making.

---

# 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=somayehforouzandeh&show_icons=true&theme=transparent&hide_border=true"/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=somayehforouzandeh&theme=transparent&hide_border=true"/>

</div>

---

# 📫 Connect With Me

- GitHub: [somayehforouzandeh](https://github.com/somayehforouzandeh)
- LinkedIn: [Somayeh Forouzandeh](https://www.linkedin.com/in/somayeh-forouzandeh)

---

> "Data is not just numbers; it is the foundation for better decisions, smarter processes, and sustainable improvement."
